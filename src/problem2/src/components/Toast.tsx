...
const Toast = ({ toast }: ToastProps) => {
  ...
  useEffect(() => {
    ...
    let autoRemoveTimer: number;
    let progressInterval: number;

    if (!isPaused && (toast.duration || 5000) > 0) {
  ...
  const handleClose = () => {
    ...
    setIsLeaving(true);
    // Wait for transition to complete before removing from context
    setTimeout(() => {
  ...
  return (
    <div
      className={`
        ...
        ${getToastStyles()}
        rounded-lg p-4 mb-3 backdrop-blur-sm
        max-w-sm w-full
        origin-right
      ...
      style={{
        ...
        paddingTop: isLeaving ? "0px" : "16px",
        paddingBottom: isLeaving ? "0px" : "16px",
    ...
    >
      ...
      <div className="flex items-start">
        ...
        <button
          onClick={handleClose}
          className="flex-shrink-0 ml-4 rounded-md inline-flex text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-gray-300 transition-colors duration-200"
        >
